...
def test_rf_model(X, y, nIters=10, numThreads=1):
    ...
    sss = StratifiedShuffleSplit(n_splits=nIters,
                                 test_size=0.2,
                                 random_state=0xf00d)
    scores = []
    kappas = []
...
rocs, scores, kappas, balanced = test_rf_model(fps,
                                               binned,
...
if doBaseline:
    outfn += 'mfp0_'
if not isBalanced:
    outfn += 'nogap_ml_results_imbalanced'
else:
    outfn += 'nogap_ml_results_balanced'

with gzip.open(f'{outfn}/{target}_randomsplit.pkl.gz', 'wb+') as outf:
