...
def run_it_oob_optimization(oob_probs,
                            labels_train,
                            thresholds,
                            ThOpt_metrics='Kappa'):
    ...
    # Optmize the decision threshold based on the ROC-curve
        ...
        fpr, tpr, thresholds_roc = metrics.roc_curve(labels_train,
                                                     oob_probs,
                                                     pos_label=1)
        specificity = 1 - fpr
...
def test_rf_model(X_train, y_train, X_test, y_test, numThreads=1):
    ...
    thresh = run_it_oob_optimization(oobProbs, y_train,
...
parser.add_argument("--baseline", default=False,
                    action="store_true")  # whether or not to use MFP0
...
if not isBalanced:
    loc = 'nogap_alt_results_imbalanced'
    out_loc = 'nogap_alt_moo_ml_results_imbalanced'
else:
    loc = 'nogap_alt_results_balanced'
    out_loc = 'nogap_alt_moo_ml_results_balanced'

fns = glob.glob(f'./{loc}/{target}_CLUSTERS_SPLIT.500.*.pkl.gz')
for fn in fns:
    rocs = []
    scores = []
    kappas = []
    ...
    with gzip.open(fn, 'rb') as inf:
        ...
        for i, best in enumerate(bscores[:10]):
            ...
            try:
                trocs, tscores, tkappas, tbalanced = test_rf_model(
                    X_train, y_train, X_test, y_test, numThreads=num_threads)
                rocs += trocs
                mdlscores += tscores
                kappas += tkappas
                balanced += tbalanced
        ...
        if doBaseline:
            ...
            prefix = ''
        outn = f'./{out_loc}/' + prefix + os.path.basename(fn).replace(
            'pkl.gz', suffix)
